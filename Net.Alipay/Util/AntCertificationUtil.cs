...
namespace Net.Alipay.Util
{
    ...
    public static class AntCertificationUtil
    {
        ...
        /// <param name="signType">算法类型，可指定RSA、RSA2或SM2</param>
        ...
        public static string GetRootCertSN(string rootCertContent, string signType)
        {
            ...
            try
            {
                ...
                {
                    //只提取与指定算法类型匹配的证书的序列号
                    if (signType.StartsWith("RSA", StringComparison.Ordinal) &&
                        cert.SigAlgOid.StartsWith("1.2.840.113549.1.1", StringComparison.Ordinal) ||
                        signType.Equals("SM2", StringComparison.Ordinal) &&
                        cert.SigAlgOid.StartsWith("1.2.156.10197.1.501", StringComparison.Ordinal))
                    {
